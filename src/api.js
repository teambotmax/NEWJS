...
class LineAPI {
  ...
  setTHttpClient(options = {
    ...
    //options.headers['X-Line-Application'] = 'CHROMEOS\t2.5.1\tChrome_OS\t10.12.5';
    options.headers['X-Line-Application'] = 'IOSIPAD 8.9.1 iPhone OS 10.12.5';
    //options.headers['X-Line-Application'] = 'DESKTOPMAC\t5.3.3-YOSEMITE-x64\tMAC\t10.12.5';
      ...
      this._authService.loginZ(reqx,(err,success) => {
        ...
        const authToken = config.tokenn;
          const certificate = config.certificate;
                ...
                this.options.path = this.config.LINE_COMMAND_PATH;
                this.setTHttpClient(this.options);
          this.options.headers['User-Agent'] = 'Line/8.9.1';
          this.axz = true;
          this.setTHttpClient(this.options);
          this.axz = false;
